...
const EventCard = ({ event, expandedComments, onToggleComments, onEditComment, onDeleteComment, onSaveCommentEdit, user, comments, onCommentChange, onCommentSubmit, onRatingChange, onRatingSubmit, onNewCommentChange }) => {
  return (
    <Card sx={{ mb: 4 }}>
      <CardContent sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Box sx={{ flex: '1 1 35%', textAlign: 'right' }}>
          {event.location_name && <Typography variant="body2" sx={{ fontWeight: 'bold' }}>Location: {event.location_name}</Typography>}
          ...
          <Typography variant="body2" sx={{ mt: 1 }}>{event.address || 'N/A'}</Typography>
        </Box>
      </CardContent>
