...
app.post("/register", async (req, res) => {
  const { username, password, firstName, lastName, role, email, universityId } =
    req.body;
...
app.post("/universities", async (req, res) => {
  ...
  if (!name || !location || !numberOfStudents) {
    return res
      .status(400)
      .json({ error: "Name, location, and number of students are required." });
  }
...
app.post("/rsos", async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .json({
        error:
          "All fields are required and at least 5 student emails are needed.",
      });
  }
  ...
  try {
    ...
    const sameDomainEmails = emailDomains.filter(
      (domain) => domain === currentUserDomain
    );

    if (sameDomainEmails.length < 5) {
      throw new Error(
        "At least 5 members with the same email domain as the admin are required to create an RSO."
      );
    }
    ...
    const [rsoResult] = await connection.execute(insertRSOQuery, [
      name,
      description,
      universityId,
      adminId,
    ...
    const adminEmail = await connection.execute(
      "SELECT email FROM users WHERE id = ?",
      [adminId]
    );
    const filteredEmails = studentEmails.filter(
      (email) => email !== adminEmail[0].email
    );
    ...
    res
      .status(201)
      .json({ message: "RSO created successfully and members added.", rsoId });
  } catch (err) {
...
app.post("/rso_memberships", async (req, res) => {
  ...
  try {
    ...
    if (membership.length > 0) {
      return res
        .status(400)
        .json({ error: "User is already a member of this RSO." });
    }
...
app.put("/events/:eventId/comments/:commentId", async (req, res) => {
  ...
  try {
    ...
    if (existingComment.length === 0) {
      return res
        .status(404)
        .json({ error: "Comment not found or does not belong to the user" });
    }
